...
by hand, door-to-door. He sends them down a street with infinite
...
make up for it, they decide to deliver presents equal to eleven times their
...
def main():
    ...
    with open(INFILE) as f:
        ...
        input = int(lines[0].strip())

    for house in range(1, input + 1):
        if not p1house:
            presents = sum(list(divisors(house))) * 10
            if presents >= input:
                p1house = house

        if not p2house:
            divs = divisors(house)
            presents = sum([d for d in divs if house / d <= 50]) * 11
            if presents >= input:
                p2house = house

        if p1house and p2house:
            break

    msg = '[Python]  Puzzle 20-1: {}'
    print(msg.format(p1house))

    msg = '[Python]  Puzzle 20-2: {}'
    print(msg.format(p2house))
